...
| Role                          | Runtime / model                                                                                                       | Effort (Claude / Codex) | Fallback                                                                        | Notes                                                                                                                                                                                                        |
| ----------------------------- | --------------------------------------------------------------------------------------------------------------------- | ----------------------- | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Scouts                        | Codex `gpt-6-luna`                                                                                                    | `xhigh`                 | —                                                                               | read-only; Orca terminals in `<WT>`; one report per lens                                                                                                                                                     |
| Plan fact check               | Codex `gpt-6-luna`                                                                                                    | `xhigh`                 | —                                                                               | read-only; Orca terminal in `<WT>`; verifies checkable plan claims only; runs before critique and again after it when critique changed the plan                                                              |
| Acceptance check              | Claude `opus`                                                                                                         | `high`                  | `Codex gpt-6-sol` at `high`                                                     | read-only; Orca terminal in `<WT>`; verifies each acceptance criterion against the integrated HEAD; one report per pass                                                                                      |
| Planner, `low`/`medium` tier  | Claude `opus`                                                                                                         | `high`                  | coordinator drafts the plan                                                     | single planner; a complete plan from the brief; writes only its own draft file                                                                                                                               |
| Planners, `high`/`xhigh` tier | Claude `opus` + Codex `gpt-6-astra`                                                                                   | `xhigh` all             | continue with the surviving planner                                             | independent complete plans from the brief; each writes only its own draft file                                                                                                                               |
| Plan critics                  | Claude `fable` + Codex `gpt-6-sol`                                                                                    | `xhigh` all             | continue with the surviving critic                                              | both, one round only; critique adversarially; read-only by instruction                                                                                                                                       |
| Builder, `low` complexity     | Claude `opus`                                                                                                         | `low`                   | `Codex gpt-6-astra` at `low`                                                    |                                                                                                                                                                                                              |
| Builder, `medium`             | Claude `opus`                                                                                                         | `medium`                | `Codex gpt-6-astra` at `medium`                                                 | default builder                                                                                                                                                                                              |
| Builder, `high`               | Claude `opus`                                                                                                         | `high`                  | `Codex gpt-6-astra` at `high`                                                   | many-file or mechanically hard, but fully specified by plan + contracts                                                                                                                                      |
| Builder, `xhigh`              | Claude `opus`                                                                                                         | `xhigh`                 | `Codex gpt-6-astra` at `xhigh`                                                  | the remaining reasoning is the risk: see the task complexity rubric                                                                                                                                          |
| Code reviewers                | Claude `opus` + Codex `gpt-6-astra`                                                                                   | `high` / `high`         | `Claude fable` for `opus`; `Codex gpt-6-sol` for `gpt-6-astra`; both at `high`  | both each round; round cap from `run_complexity`                                                                                                                                                             |
| Security reviewers            | Claude `opus` + Codex `gpt-6-astra`                                                                                   | `high` / `high`         | `Claude fable` for `opus`; `Codex gpt-6-sol` for `gpt-6-astra`; both at `high`  | `medium` and above only; both in round 1, dispatched alongside the code reviewers; later rounds only on the Phase 7 security trigger; each runs the mapped security-review skill (`references/skill-map.md`) |
| Adversarial QA                | Cross-model to the builder of the highest-complexity merged task: Claude → Codex `gpt-6-astra`; Codex → Claude `opus` | `high` / `high`         | `Claude fable` for `opus`; `Codex gpt-6-sol` for `gpt-6-astra`; both at `high`  | `medium`/`high`/`xhigh` runs only; runs once after all code-review rounds and fixes; disposable worktree, branch never merged                                                                                |


If a worker fails to boot or run, use the listed fallback model and effort.
...
| `medium` | Local decisions inside one area     | one module or layer, established conventions; more modules only when the changes are simple with a small blast radius: a new endpoint or component following existing shapes, a bugfix needing investigation within one subsystem; changes stay behind existing interfaces; moving a contract other modules depend on means `high` or above                                                                    |
...
- Use `xhigh` when difficult reasoning or ambiguity remains during implementation.
...
- Auth/authz, payments, destructive data migration, security-critical logic, and concurrency or consistency primitives: never below `high`. Use `xhigh` when the approach is not pinned.
...
```bash
nono run --profile my-claude --allow-cwd --allow-gpu --allow "<RUNDIR>" -- claude --model <opus|fable> --effort <effort> --permission-mode bypassPermissions
```

**Codex in `<WT>`** (scout, plan fact check, planner, critic, reviewer):

```bash
ncodex --model <gpt-6-luna|gpt-6-astra|gpt-6-sol> -c 'model_reasoning_effort="<effort>"' --sandbox danger-full-access --ask-for-approval never
```

**Codex in a task or QA worktree** (builder, fix, QA):

```bash
nono run --profile my-codex --allow-cwd --allow-gpu --allow "<RUNDIR>" -- codex --model <gpt-6-astra|gpt-6-sol> -c 'model_reasoning_effort="<effort>"' --sandbox danger-full-access --add-dir "<RUNDIR>" --ask-for-approval never
```
