...
- `<QA_HEAD>`;
- `<RUNDIR>`;
- `<SYNTHESIS_PATHS>`: every completed `<RUNDIR>/review/synthesis-r*.md`, one per line.
...
Write the full report to <RUNDIR>/review/qa-findings.md. For each finding, include a reproducible test scenario or screenshot. When no finding holds, still report what you attacked, how you tested it, and that it held.
