...
Test the current branch against `origin/main`, unless the user specifies a different base ref (a branch, tag, or commit SHA) or a full comparison range, in which case use that. Assume bugs exist and hunt for them. Output ONLY concrete, reproducible findings: no summaries, praise, opinions, or process narration.
...
Let `BASE` = the user-specified base ref, or `origin/main` if none was given. If the user gave a full comparison range, use it verbatim wherever a `$BASE` range appears below.
...
3. **Map the affected surface**: identify every affected user-facing path (routes, endpoints, forms, buttons, state transitions). Note which are new vs modified. Trace from changed functions to their callers and entry points.
4. **Determine project tooling**: find the build command, dev server start command, and dev server URL. Check `package.json`, `Makefile`, `Cargo.toml`, or equivalent. Note whether the project has a test runner configured; if it does, note the command for the Phase 3 executors. If not (e.g., no `test` script in `package.json`, or it's the default `echo "Error: no test specified"`), the Phase 3 executors run standalone scripts directly (e.g., `node`, `npx tsx`, `python`).

Then launch scouts in parallel as read-only subagents (Explore in Claude Code), model per the Subagent models table:
...
**You plan the tests. Subagents execute them.** Dispatch subagents to write and run the code-level test cases from your prioritized plan. Each subagent gets: the specific vectors to test, the target functions/files, the test runner command, and the reporting format below (model per the Subagent models table). Subagents delete their throwaway test files after capturing results. Launch subagents in parallel where tests are independent.
...
Start the dev server if not already running. First, independently verify the changed flows end-to-end through the browser; do NOT trust earlier verification. Navigate to the feature, use it, confirm it actually works before trying to break it.

Then launch subagents with agent-browser in headless mode for the browser vectors from your prioritized plan. Each subagent gets: the dev server URL, the specific pages/flows to test, the test plan, and the reporting format below (model per the Subagent models table). Give the subagents disjoint pages/flows; run vectors sequentially when they touch the same mutable state, so one subagent's double-submit or multi-tab test cannot contaminate another's flow.
...
Number findings sequentially (#1, #2, ...). For each finding, output a block with exactly these fields:
...
- Finding: {what's broken, in one line}
...
- Regression test: {for bug findings: the exact test input/scenario that reproduces the bug, phrased as a locked test case a developer can promote into the permanent test suite. Omit for Test gap findings, where the missing test is itself the finding.}

If the same issue recurs, report it once and reference it later as "See issue #N".
